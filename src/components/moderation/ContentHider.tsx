...
function ContentHiderActive({
  ...
  // Always set override to false and never allow changing it for NSFW content
  ...
  // Check if this is NSFW content - if so, always enforce no-override
  const isNSFWContent = React.useMemo(() => {
    if (!blur || blur.type !== 'label') return false
    
    // Check for NSFW-related labels
    const nsfw_labels = ['porn', 'sexual', 'nudity', 'nsfw', 'graphic', 'gore']
    return nsfw_labels.includes(blur.label.val)
  }, [blur])
  
  // For NSFW content, force noOverride to be true
  if (isNSFWContent) {
    ...
    <View testID={testID} style={[a.overflow_hidden, style]}>
      ...
      <Button
        onPress={e => {
          e.preventDefault()
          e.stopPropagation()
          // For NSFW content or when noOverride is true, only allow viewing details
          if (modui.noOverride) {
            control.open()
          } else {
            setOverride(v => !v)
